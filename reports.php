...
// This Year's Data
$sql_ty = "SELECT MONTHNAME(res_date) as month, COUNT(reservation_id) as count 
           FROM reservations 
           WHERE res_date BETWEEN ? AND ? 
           GROUP BY MONTH(res_date), MONTHNAME(res_date)
           ORDER BY MONTH(res_date)";
...
// Last Year's Data (for the same date range, but one year prior)
$startDateLY = date('Y-m-d', strtotime('-1 year', strtotime($startDate)));
$endDateLY = date('Y-m-d', strtotime('-1 year', strtotime($endDate)));
$sql_ly = "SELECT MONTHNAME(res_date) as month, COUNT(reservation_id) as count 
           FROM reservations 
           WHERE res_date BETWEEN ? AND ? 
           GROUP BY MONTH(res_date), MONTHNAME(res_date)
           ORDER BY MONTH(res_date)";
...
// NOTE FOR DEVELOPER: This query assumes you have a `source` column in your reservations table.
// If you don't, the CASE statement provides a simulation based on reservation ID.
$sql_source = "SELECT 
                CASE 
...
$new_guests = 0;
$returning_guests = 0;
// Get all unique guests (by email) within the selected date range
...
if ($stmt_guests = mysqli_prepare($link, $sql_guests_in_range)) {
    ...
    if (!empty($guests_in_range)) {
        // For each unique guest, check if they had any reservation *before* the current start date
        ...
        if ($stmt_check = mysqli_prepare($link, $sql_check_prior)) {
            foreach ($guests_in_range as $guest_email) {
                ...
                if (mysqli_stmt_num_rows($stmt_check) > 0) {
                    $returning_guests++; // If they have a past reservation, they are "returning"
                } else {
                    $new_guests++; // If they have no past reservations, they are "new"
                }
            }
...
// --- Data for KPI Summary Cards (based on filtered date range) ---
$sql_total = "SELECT COUNT(reservation_id) as total FROM reservations WHERE res_date BETWEEN ? AND ? AND deleted_at IS NULL";
$total_reservations = 0;
if($stmt_total = mysqli_prepare($link, $sql_total)){
    mysqli_stmt_bind_param($stmt_total, "ss", $startDate, $endDate);
    mysqli_stmt_execute($stmt_total);
    $result_total = mysqli_stmt_get_result($stmt_total);
    $total_reservations = mysqli_fetch_assoc($result_total)['total'] ?? 0;
}


$busiest_month = 'N/A';
$sql_kpi = "SELECT MONTHNAME(res_date) as month FROM reservations 
            WHERE res_date BETWEEN ? AND ? AND deleted_at IS NULL
            GROUP BY MONTH(res_date), MONTHNAME(res_date) 
            ORDER BY COUNT(reservation_id) DESC LIMIT 1";
if ($stmt_kpi = mysqli_prepare($link, $sql_kpi)) {
    mysqli_stmt_bind_param($stmt_kpi, "ss", $startDate, $endDate);
    mysqli_stmt_execute($stmt_kpi);
    $result_kpi = mysqli_stmt_get_result($stmt_kpi);
    if ($row_kpi = mysqli_fetch_assoc($result_kpi)) {
        $busiest_month = substr($row_kpi['month'], 0, 3);
    }
...
<script>
    // This data is now generated by the updated PHP code based on your database and filters.
    const reportData = {
        pacing: {
